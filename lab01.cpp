...
void gamma_change(double gamma)
{
	...
	for (int i = 0; i < 256; i++) //расчет переходной матрицы интенсивности
	{
		q = i; q /= 255;
		new_intensive[i] = pow(q, gamma) * 255;
	}

	for (int i = 0; i < gamma_graph.rows; i++) //отображение переходного графа
	{
		circle(gamma_graph, Point(i, 256 - new_intensive[i]), 0, Scalar(100), 2);
	}

	for (int i = 0; i < initial_picture.rows; i++) //изменение интенсивности выходной картинки
...
void sigmoida_change(double alpha, double beta)
{
	...
	for (int i = 0; i < sigmoida_graph.rows; i++) //отображение переходного графа
	{
